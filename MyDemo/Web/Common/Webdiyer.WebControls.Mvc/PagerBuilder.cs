    ...
    public class PagerBuilder
    {
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<a disabled=\"disabled\"  style='color:#bababa'>{0}</a>", item.Text));
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? String.Format("<a href=\"javascript:void(0)\" style='color:#bababa' >{0}</a>", item.Text)  // ? HttpUtility.HtmlEncode(item.Text) --by:hyx update:2016-11-16 
