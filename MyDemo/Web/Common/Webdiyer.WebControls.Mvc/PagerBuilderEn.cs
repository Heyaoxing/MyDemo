...
namespace Webdiyer.WebControls.Mvc
{
    ...
    public class PagerBuilderEn
    {
        ...
        private readonly PagerOptionsEn _pagerOptionsEn;
        ...
        /// 适用于PagedList为null时
        /// </summary>
        internal PagerBuilderEn(HtmlHelper html, AjaxHelper ajax, PagerOptionsEn pagerOptionsEn, IDictionary<string, object> htmlAttributes)
        {
            if (pagerOptionsEn == null)
                pagerOptionsEn = new PagerOptionsEn();
            _html = html;
            _ajax = ajax;
            _pagerOptionsEn = pagerOptionsEn;
            _htmlAttributes = htmlAttributes;
        }
        internal PagerBuilderEn(HtmlHelper html, AjaxHelper ajax, string actionName, string controllerName,
            int totalPageCount, int pageIndex, PagerOptionsEn pagerOptions, string routeName, RouteValueDictionary routeValues,
            MvcAjaxOptions ajaxOptions, IDictionary<string, object> htmlAttributes)
        {
            _ajaxPagingEnabled = (ajax != null);
            if (pagerOptions == null)
                pagerOptions = new PagerOptionsEn();
            _html = html;
            _ajax = ajax;
            ...
            if (_startPageIndex < 1)
                _startPageIndex = 1;
            // end page index
            _endPageIndex = _startPageIndex + _pagerOptionsEn.NumericPagerItemCount - 1;
            if (_endPageIndex > _totalPageCount)
                _endPageIndex = _totalPageCount;
        }
        //non Ajax pager builder
        internal PagerBuilderEn(HtmlHelper helper, string actionName, string controllerName, int totalPageCount,
            int pageIndex, PagerOptionsEn pagerOptions, string routeName, RouteValueDictionary routeValues,
        ...
        //Ajax pager builder
        internal PagerBuilderEn(AjaxHelper helper, string actionName, string controllerName, int totalPageCount,
            int pageIndex, PagerOptionsEn pagerOptions, string routeName, RouteValueDictionary routeValues,
            MvcAjaxOptions ajaxOptions, IDictionary<string, object> htmlAttributes)
        ...
        {
            var item = new PagerItem(_pagerOptionsEn.PrevPageText, _pageIndex - 1, _pageIndex == 1, PagerItemType.PrevPage);
            if (!item.Disabled || (item.Disabled && _pagerOptionsEn.ShowDisabledPagerItems))
                results.Add(item);
        }
        private void AddFirst(ICollection<PagerItem> results)
        {
            var item = new PagerItem(_pagerOptionsEn.FirstPageText, 1, _pageIndex == 1, PagerItemType.FirstPage);
        ...
        {
            if (_startPageIndex > 1 && _pagerOptionsEn.ShowMorePagerItems)
            {
                var index = _startPageIndex - 1;
                if (index < 1) index = 1;
                var item = new PagerItem(_pagerOptionsEn.MorePageText, index, false, PagerItemType.MorePage);
                results.Add(item);
            }
        }
        private void AddPageNumbers(ICollection<PagerItem> results)
        {
            for (var pageIndex = _startPageIndex; pageIndex <= _endPageIndex; pageIndex++)
            {
                ...
                if (pageIndex == _pageIndex && !string.IsNullOrEmpty(_pagerOptionsEn.CurrentPageNumberFormatString))
                    text = String.Format(_pagerOptionsEn.CurrentPageNumberFormatString, text);
                else if (!string.IsNullOrEmpty(_pagerOptionsEn.PageNumberFormatString))
                    text = String.Format(_pagerOptionsEn.PageNumberFormatString, text);
        ...
        private void AddMoreAfter(ICollection<PagerItem> results)
        {
            if (_endPageIndex < _totalPageCount)
            {
                var index = _startPageIndex + _pagerOptionsEn.NumericPagerItemCount;
        ...
        {
            var item = new PagerItem(_pagerOptionsEn.NextPageText, _pageIndex + 1, _pageIndex >= _totalPageCount, PagerItemType.NextPage);
            if (!item.Disabled || (item.Disabled && _pagerOptionsEn.ShowDisabledPagerItems))
                results.Add(item);
        }
        private void AddLast(ICollection<PagerItem> results)
        {
            var item = new PagerItem(_pagerOptionsEn.LastPageText, _totalPageCount, _pageIndex >= _totalPageCount, PagerItemType.LastPage);
            if (!item.Disabled || (item.Disabled && _pagerOptionsEn.ShowDisabledPagerItems))
                results.Add(item);
        }
        /// <summary>
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            var pageValue = viewContext.RouteData.Values[_pagerOptionsEn.PageIndexParameterName];
            string routeName = _routeName;
            // 设置Route Value中的分页导航Url参数值，pageIndex为0时生成适用于脚本的导航链接
            if (pageIndex == 0)
                routeValues[_pagerOptionsEn.PageIndexParameterName] = "__" + _pagerOptionsEn.PageIndexParameterName + "__";
            else
            {
                if (pageIndex == 1)
                {
                    if (!string.IsNullOrWhiteSpace(_pagerOptionsEn.FirstPageRouteName))
                    //如果显式指定了FirstPageRouteName，则使用此Route
                    {
                        routeName = _pagerOptionsEn.FirstPageRouteName;
                        routeValues.Remove(_pagerOptionsEn.PageIndexParameterName); //去除页索引参数
                        viewContext.RouteData.Values.Remove(_pagerOptionsEn.PageIndexParameterName);
                    }
                    else
                    {
                        ...
                        if (curRoute != null &&
                            (curRoute.Defaults[_pagerOptionsEn.PageIndexParameterName] == UrlParameter.Optional ||
                             !curRoute.Url.Contains("{" + _pagerOptionsEn.PageIndexParameterName + "}")))
                        {
                            routeValues.Remove(_pagerOptionsEn.PageIndexParameterName); //去除页索引参数
                            viewContext.RouteData.Values.Remove(_pagerOptionsEn.PageIndexParameterName);
                        }
                        ...
                        {
                            routeValues[_pagerOptionsEn.PageIndexParameterName] = pageIndex;
                        }
                    }
                }
                else
            ...
            if (!string.IsNullOrEmpty(routeName))
                ...
                url = UrlHelper.GenerateUrl(null, _actionName, _controllerName, routeValues, routes,
                                             viewContext.RequestContext, false);
            if (pageValue != null)
                viewContext.RouteData.Values[_pagerOptionsEn.PageIndexParameterName] = pageValue;
            return url;
        }
        /// <summary>
        /// 生成最终的分页Html代码
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            if (_totalPageCount <= 1 && _pagerOptionsEn.AutoHide)
            ...
            if ((_pageIndex > _totalPageCount && _totalPageCount > 0) || _pageIndex < 1)
            {
                return
                    MvcHtmlString.Create(string.Format("{0}<div style=\"color:red;font-weight:bold\">{1}</div>{0}",
                                         CopyrightText, _pagerOptionsEn.PageIndexOutOfRangeErrorMessage));
            }

            var pagerItems = new List<PagerItem>();
            //First page
            if (_pagerOptionsEn.ShowFirstLast)
                AddFirst(pagerItems);
            // Prev page
            if (_pagerOptionsEn.ShowPrevNext)
                AddPrevious(pagerItems);
            if (_pagerOptionsEn.ShowNumericPagerItems)
            {
                if (_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _startPageIndex > 1)
                    pagerItems.Add(new PagerItem("1", 1, false, PagerItemType.NumericPage));
                // more page before numeric page buttons
                if (_pagerOptionsEn.ShowMorePagerItems && ((!_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _startPageIndex > 1) || (_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _startPageIndex > 2)))
                    AddMoreBefore(pagerItems);
                // numeric page
                AddPageNumbers(pagerItems);
                // more page after numeric page buttons
                if (_pagerOptionsEn.ShowMorePagerItems && ((!_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _endPageIndex < _totalPageCount) || (_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _totalPageCount > _endPageIndex + 1)))
                    AddMoreAfter(pagerItems);
                if (_pagerOptionsEn.AlwaysShowFirstLastPageNumber && _endPageIndex < _totalPageCount)
            ...
            var tb = new TagBuilder(_pagerOptionsEn.ContainerTagName);
            if (!string.IsNullOrEmpty(_pagerOptionsEn.Id))
                tb.GenerateId(_pagerOptionsEn.Id);
            if (!string.IsNullOrEmpty(_pagerOptionsEn.CssClass))
                tb.AddCssClass(_pagerOptionsEn.CssClass);
            if (!string.IsNullOrEmpty(_pagerOptionsEn.HorizontalAlign))
            {
                string strAlign = "text-align:" + _pagerOptionsEn.HorizontalAlign.ToLower();
            ...
            if (_pagerOptionsEn.ShowPageIndexBox)
            {
                if (!_ajaxPagingEnabled)
                {
                ...
                sb.Length -= _pagerOptionsEn.PagerItemsSeperator.Length;
        ...
        private void AddDataAttributes(IDictionary<string, object> attrs)
        {
            ...
            attrs.Add("data-pageparameter", _pagerOptionsEn.PageIndexParameterName);
            attrs.Add("data-maxpages", _totalPageCount);
            if (_pagerOptionsEn.ShowPageIndexBox && _pagerOptionsEn.PageIndexBoxType == PageIndexBoxType.TextBox)
            {
                attrs.Add("data-outrangeerrmsg", _pagerOptionsEn.PageIndexOutOfRangeErrorMessage);
                attrs.Add("data-invalidpageerrmsg", _pagerOptionsEn.InvalidPageIndexErrorMessage);
            }
        }
        private string BuildGoToPageSection()
        {
            var piBuilder = new StringBuilder();
            if (_pagerOptionsEn.PageIndexBoxType == PageIndexBoxType.DropDownList)
            {
                // start page index
                int startIndex = _pageIndex - (_pagerOptionsEn.MaximumPageIndexItems / 2);
                if (startIndex + _pagerOptionsEn.MaximumPageIndexItems > _totalPageCount)
                    startIndex = _totalPageCount + 1 - _pagerOptionsEn.MaximumPageIndexItems;
                if (startIndex < 1)
                    startIndex = 1;
                // end page index
                int endIndex = startIndex + _pagerOptionsEn.MaximumPageIndexItems - 1;
                if (endIndex > _totalPageCount)
                    endIndex = _totalPageCount;
                piBuilder.AppendFormat("<select data-pageindexbox=\"true\"{0}>", (_pagerOptionsEn.ShowGoButton ? "" : " data-autosubmit=\"true\""));
                ...
                piBuilder.AppendFormat(
                    "<input type=\"text\" value=\"{0}\" data-pageindexbox=\"true\"{1}/>", _pageIndex, (_pagerOptionsEn.ShowGoButton ? "" : " data-autosubmit=\"true\""));
            string outHtml;
            if (!string.IsNullOrEmpty(_pagerOptionsEn.PageIndexBoxWrapperFormatString))
            {
                outHtml = string.Format(_pagerOptionsEn.PageIndexBoxWrapperFormatString, piBuilder);
                piBuilder = new StringBuilder(outHtml);
            }
            if (_pagerOptionsEn.ShowGoButton)
            {
                piBuilder.AppendFormat("<input type=\"button\" data-submitbutton=\"true\" value=\"{0}\"/>", _pagerOptionsEn.GoButtonText);
            }
            if (!string.IsNullOrEmpty(_pagerOptionsEn.GoToPageSectionWrapperFormatString) ||
                !string.IsNullOrEmpty(_pagerOptionsEn.PagerItemWrapperFormatString))
            {
                outHtml = string.Format(
                    _pagerOptionsEn.GoToPageSectionWrapperFormatString ?? _pagerOptionsEn.PagerItemWrapperFormatString,
                    piBuilder);
            }
            else
        ...
        private string GenerateAjaxAnchor(PagerItem item)
        {
            ...
            if (string.IsNullOrWhiteSpace(url))
                return "<a class='active' style='z-index:2;color:#fff; cursor: default;background-color: #428bca;  border-color: #428bca;' disabled='disabled'>" + item.Text + "</a>";
        ...
        private MvcHtmlString CreateWrappedPagerElement(PagerItem item, string el)
        {
            ...
            switch (item.Type)
            {
                ...
                case PagerItemType.PrevPage:
                    if ((!string.IsNullOrEmpty(_pagerOptionsEn.NavigationPagerItemWrapperFormatString) ||
                         !string.IsNullOrEmpty(_pagerOptionsEn.PagerItemWrapperFormatString)))
                        navStr =
                            string.Format(
                                _pagerOptionsEn.NavigationPagerItemWrapperFormatString ??
                                _pagerOptionsEn.PagerItemWrapperFormatString, el);
                    break;
                case PagerItemType.MorePage:
                    if ((!string.IsNullOrEmpty(_pagerOptionsEn.MorePagerItemWrapperFormatString) ||
                         !string.IsNullOrEmpty(_pagerOptionsEn.PagerItemWrapperFormatString)))
                        navStr =
                            string.Format(
                                _pagerOptionsEn.MorePagerItemWrapperFormatString ??
                                _pagerOptionsEn.PagerItemWrapperFormatString, el);
                    break;
                case PagerItemType.NumericPage:
                    if (item.PageIndex == _pageIndex &&
                        (!string.IsNullOrEmpty(_pagerOptionsEn.CurrentPagerItemWrapperFormatString) ||
                         !string.IsNullOrEmpty(_pagerOptionsEn.PagerItemWrapperFormatString))) //current page
                        navStr =
                            string.Format(
                                _pagerOptionsEn.CurrentPagerItemWrapperFormatString ??
                                _pagerOptionsEn.PagerItemWrapperFormatString, el);
                    else if (!string.IsNullOrEmpty(_pagerOptionsEn.NumericPagerItemWrapperFormatString) ||
                             !string.IsNullOrEmpty(_pagerOptionsEn.PagerItemWrapperFormatString))
                        navStr =
                            string.Format(
                                _pagerOptionsEn.NumericPagerItemWrapperFormatString ??
                                _pagerOptionsEn.PagerItemWrapperFormatString, el);
                    break;
            }
            return MvcHtmlString.Create(navStr + _pagerOptionsEn.PagerItemsSeperator);
        }
        private void AddQueryStringToRouteValues(RouteValueDictionary routeValues, ViewContext viewContext)
        {
            ...
            {
                var invalidParams = new[] { "x-requested-with", "xmlhttprequest", _pagerOptionsEn.PageIndexParameterName.ToLower() };
